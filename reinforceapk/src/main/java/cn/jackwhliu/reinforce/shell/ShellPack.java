package cn.jackwhliu.reinforce.shell;
...
public class ShellPack {
     ...
     * @param dir
     */
    public void rewriteManifest(ManifestInfo hostInfo, String dir) throws Exception {
        ...
        String replace = "package=\"" + hostInfo.packageName + "\"";
        ...
        String content = hostInfo.content.toString().replace("@style/lttransparent", "@android:style/Theme.Translucent.NoTitleBar.Fullscreen");
        if (hostInfo.applicationName != null && !hostInfo.applicationName.isEmpty()) {
            split[1] = split[1].replace("null", hostInfo.applicationName);
        }
        sb.append(split[0]);
        sb.append(hostInfo.features);
    ...
    public void copyLauncherIcon(String hostIconRes) throws Exception {
        String iconName = hostIconRes.substring(hostIconRes.indexOf("/") + 1) + ".";
        List<File> drawableDirs = FileUtils.getSubFiles(Utils.HOST_APK_UNPACK_PATH + "/res", "drawable");
        if (drawableDirs == null || drawableDirs.size() == 0) {
            throw new Exception("host apk drawable dir is null");
        }
    ...
    public void changeAppName(String hostAppNameRes) throws Exception {
        String appResName = hostAppNameRes.substring(hostAppNameRes.indexOf("/") + 1);
        String appName = FileUtils.readStringRes(Utils.HOST_APK_STRING_RES_PATH, appResName);
        System.out.println("host apk app name: " + appName);
        if (appName == null || appName.isEmpty()) {
            throw new Exception("read host apk app name failed, app name is empty");
        }
    ...
    public void changeVersion(String hostVerCode, String hostVerName) throws Exception {
        ...
        apktoolContent = apktoolContent.replace(versionCode, versionCodeStr + hostVerCode);
        apktoolContent = apktoolContent.replace(name, versionNameStr + hostVerName);
    ...
    private void buildSignKey(String packageName) throws Exception {
        ...
        keystorePass = Utils.encrypt(packageName + System.currentTimeMillis());
...
//        }
    ...
    public void copyHostApkToDex() throws Exception {
        ...
        System.arraycopy(Utils.intToBytes(apkFileLen), 0, newDexBytes, totalLength - 4, 4);
    ...
    private void fixFileSizeHead(byte[] dexBytes) {
        byte[] fileLenBytes = Utils.intToBytes(dexBytes.length);
    ...
    private void fixCheckSumHead(byte[] dexBytes) {
        ...
        int va = (int) value;
        byte[] vaBytes = Utils.intToBytes(va);
        byte[] trans = new byte[4];
        for (int i = 0; i < 4; i++) {
            trans[i] = vaBytes[3 - i];
        }
